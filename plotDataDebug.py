...
# Get actual data transcribed from video
date = "0.0.0.0"
debugPath = "/home/pi/Desktop/"+date
while not os.path.isdir(debugPath):
    date = raw_input("Please enter the date and time of the test (mm.dd.hh.mm): ")
    debugPath = "/home/pi/Desktop/"+date
x_actual= open(debugPath+"/x").read().splitlines()
y_actual= open(debugPath+"/y").read().splitlines()

# Copy data files
source = os.listdir(debugPath)
for files in source:
    shutil.copy(debugPath+"/"+files,"/home/pi/Documents/ae456final/data/raw")

# Prepare data for plotting
execfile('correctData.py')
execfile('filter.py')
...
for i in range(len(y_actual)):
    y_actual[i] = float(y_actual[i])*0.9144+float(GPS_alt[0])
...
# Plot rotation data
plt.figure()
plt.plot(times[0:len(rot_x)],rot_x,'r.',label='x rotation')
plt.plot(times[0:len(rot_y)],rot_y,'g.',label='y rotation')
...
plt.plot(x_actual,y_actual,'om',label='Actual')
